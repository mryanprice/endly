...
var errorRewrites = map[string]func(util.SecretLookup, *location.Resource) string{
	"authentication failed": func(service util.SecretLookup, resource *location.Resource) string {
	...
	"error validating server certificate": func(service util.SecretLookup, resource *location.Resource) string {
		return fmt.Sprintf("failed to validate svn certificate: %v", resource.URL)
	},
	"username": func(service util.SecretLookup, resource *location.Resource) string {
		username, _ := util.GetUsername(service, resource.Credentials)
		return fmt.Sprintf("failed to authenticate username: %v with %v secret", username, resource.Credentials)
	},
}

func checkVersionControlAuthErrors(err error, service util.SecretLookup, resource *location.Resource) error {
	if err != nil {
